...
const MCOL_COLORS = ["#5C1523", "#1e40af", "#065f46"] as const;
        ...
        <div className="space-y-4 animate-in fade-in duration-300">
            ...
            <div className="rounded-2xl overflow-hidden shadow-sm border border-slate-100">
                <div className="px-5 py-3.5 flex items-center justify-between"
                    style={{ background: "linear-gradient(135deg,#5C152322 0%,#5C15230a 100%)", borderRight: "4px solid #5C1523" }}>
            ...
            {questionSections.map(section => {
                ...
                const sectionColor = section.color || "#5C1523";
                return (
                    <div key={section.id} className="rounded-2xl overflow-hidden shadow-sm border border-slate-100">
                        {/* Header */}
            ...
            <button onClick={handleSubmit} disabled={saving || saved}
                ...
                style={saved ? {} : { background: "linear-gradient(135deg,#5C1523 0%,#7A1E30 100%)" }}>
            ...
            <div className="bg-white rounded-2xl border border-qatar-gray-border qatar-card-shadow p-4">
                ...
                <div className="h-2.5 bg-slate-100 rounded-full overflow-hidden">
                    <div className="h-full rounded-full transition-all duration-700"
                        style={{ width: `${respondents.length > 0 ? Math.round((totalDone / respondents.length) * 100) : 0}%`, background: "linear-gradient(90deg,#5C1523,#7A1E30)" }} />
                </div>
                            ...
                            <button key={dept} onClick={() => setSelectedDept(dept)}
                                ...
                                <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
                                    <div className="h-full rounded-full transition-all"
                                        style={{ width: `${deptPct}%`, background: allDone ? "#10b981" : "linear-gradient(90deg,#5C1523,#7A1E30)" }} />
                                </div>
                            </button>
                        );
                    })}
    ...
    return (
        <div className="space-y-4">
            ...
            <button onClick={handleSave} disabled={saving || saved}
                ...
                style={saved ? {} : { background: "linear-gradient(135deg,#5C1523,#7A1E30)" }}>
...
// ── Analytics Tab ──────────────────────────────────────────────────────────
const MCOL_COLORS_A = ["#5C1523", "#1e40af", "#065f46"];
    ...
    const topQuestions = useMemo(() => {
        ...
        for (const { section, qStats } of sectionStats) {
            for (const qs of qStats) {
                if (qs.avg > 0) all.push({ sectionTitle: section.title, sectionColor: section.color ?? "#5C1523", q: qs.q, avg: qs.avg, cnt: qs.cnt });
            }
        }
    ...
    return (
        <div dir="rtl" className="space-y-5 animate-in fade-in duration-300">
                ...
                <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                        <button onClick={() => setSelectedDept("all")}
                            ...
                            style={selectedDept === "all" ? { background: "linear-gradient(135deg,#5C1523,#7A1E30)" } : {}}>
                            الكل
                        </button>
                        {departments.map(d => (
                            <button key={d} onClick={() => setSelectedDept(d)}
                                ...
                                style={selectedDept === d ? { background: "linear-gradient(135deg,#5C1523,#7A1E30)" } : {}}>
                                {d}
                            </button>
                        ))}
                    </div>
            ...
            {topQuestions.length > 0 && (
                <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                    <div className="px-5 py-3 border-b border-slate-100 flex items-center justify-between"
                        style={{ background: "linear-gradient(135deg,#7c2d12,#5C1523)" }}>
